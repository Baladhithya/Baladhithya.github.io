...
export function ExperienceSection() {
      ...
      (entries) => {
        if (entries[0].isIntersecting) {
          ...
          const interval = setInterval(() => {
            setTypingIndex(prev => {
              if (prev < 30) { // arbitrary max length for typing animation
                return prev + 1;
              } else {
  ...
  return (
    <section id="experience" className="py-16 md:py-20 bg-black border-t border-white/10 relative overflow-hidden">
      ...
      <div className="max-w-screen-lg mx-auto px-4 md:px-8 relative z-10">
        <div className="mb-12">
          <div className="flex items-center text-white mb-2">
            ...
            <span className="terminal-font text-white text-lg">cd experience/</span>
          </div>
        ...
        <div className="border border-white/20 bg-black rounded mb-8">
          ...
          <div className="border-b border-white/20 bg-white/5 px-4 py-2 flex justify-between items-center">
            ...
            <div className="text-white/60 text-xs font-mono">experience.log</div>
          </div>
          ...
          <div className="p-4 border-b border-white/10 font-mono text-sm">
            <div className="flex">
              ...
              <span className="text-white">
                {typingIndex >= 15 ? 'tail -f experience.log' : 'tail -f experience.log'.substring(0, typingIndex)}
              </span>
                ...
                <div key={exp.id} className="animate-fadeIn mb-8">
                      ...
                      {exp.responsibilities.map((resp, idx) => (
                        <div key={idx} className="mb-3 group">
                          <div className="flex items-baseline">
                            <span className="text-green-400 mr-2">•</span>
                            <span className="text-white/90 group-hover:text-white transition-colors">{resp}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2 ml-4 mt-2">
                      {['React', 'Node.js', 'TypeScript', 'Docker', 'Git'].map((tech, idx) => (
                        <span key={idx} className="bg-white/5 border border-white/20 px-2 py-1 text-xs text-white/80">
                          {tech}
                        </span>
                      ))}
                    </div>
